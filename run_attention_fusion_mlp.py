...
def objective(trial, full_train_dataset, config):
    ...
    fold_accuracies = []
    print(f"\nTrial {trial.number}: proj={projection_dim}, fusion_hidden={fusion_hidden_dim}, dr={dropout_rate:.2f}, lr={lr:.6f}, wd={weight_decay:.6f}")
    ...
    for fold, (train_split_idx, val_split_idx) in enumerate(skf.split(split_indices, labels_np)):
        ...
        # Ensure indices are within bounds of the original dataset
        ...
        if np.max(train_orig_idx) >= len(full_train_dataset) or np.max(val_orig_idx) >= len(full_train_dataset):
            print(f"Error: Fold {fold+1} indices out of bounds after label extraction. Skipping fold.")
            continue
...
if __name__ == '__main__':
    ...
    if model_device.type != config.device:
        ...
        # Check type again after re-attempt
        if model_device.type != config.device:
    ...
    if torchinfo:
        ...
        try:
            summary_model = AttentionFusionMLP(
                audio_dim=config.audio_dim, video_dim=config.video_dim, pers_dim=config.pers_dim,
                projection_dim=best_params['projection_dim'], fusion_hidden_dim=best_params['fusion_hidden_dim'],
