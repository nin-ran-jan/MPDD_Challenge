...
from Models.late_fusion_lstm import LateFusionLSTM
...
model_save_path = './best_late_fusion_mlp.pth' # Path to save the final best model
optuna_trials = 50
...
if __name__ == '__main__':
    ...
    if torchinfo:
        ...
        try:
            # --- Use LateFusionMLP for summary model ---
            summary_model = LateFusionLSTM(
                audio_dim=config.audio_dim,      # example for Wav2Vec
                video_dim=config.video_dim,      # example for OpenFace
                pers_dim=config.pers_dim,      # example for RoBERTa personalized
                hidden_dim_lstm=128, # LSTM hidden size
