# -*- coding: utf-8 -*-
"""
Training Script for Late Fusion Transformer Model (CLS Token Aggregation) - v2
...
- Fixes NameError for ReduceLROnPlateau.
"""
...
# *** REMOVED: Import ReduceLROnPlateau as it's not used ***
...
try:
    ...
    # Ensure this points to the CLS token version if separate file, or the correct class def below
    # from Models.late_fusion_transformer import LateFusionTransformerWithCLS
    from Utils.focal_loss import FocalLoss
    from Utils.test_val_split import train_val_split1, train_val_split2
except ImportError as e:
    print(f"Error importing required modules: {e}")
    print("Please ensure DataSets, Models, Utils directories are correctly structured and importable.")
    exit(1)
...
# --- Model Definition (CLS Token Version) ---
# (Pasting the model definition here for self-contained script)
...
class LateFusionTransformerWithCLS(nn.Module):
    ...
    def forward(self, A_feat, V_feat, P_feat):
        ...
        # TODO: Add padding mask if using variable length sequences within batch
...
def train_epoch(model, dataloader, optimizer, criterion, device, clip_grad_norm=1.0, scheduler=None):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        try:
            ...
            # Use autocast for mixed precision if desired (requires torch.cuda.amp)
            ...
            # scaler.scale(loss).backward() # If using mixed precision
            loss.backward()

            if clip_grad_norm is not None:
                # scaler.unscale_(optimizer) # If using mixed precision
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=clip_grad_norm)

            # scaler.step(optimizer) # If using mixed precision
            # scaler.update() # If using mixed precision
            optimizer.step() # Standard step

            # *** FIX: Step LR scheduler (if using step-based scheduler like cosine) ***
            if scheduler is not None:
                 # Check necessary because ReduceLROnPlateau isn't defined here
                 # This assumes schedulers needing per-step updates are NOT ReduceLROnPlateau
                 # A more robust check might be needed if other epoch-based schedulers are used
    ...
    # print(f"  Train batches processed: {processed_batches}") # Optional debug print
...
def evaluate(model, dataloader, criterion, device, num_classes):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            try:
                ...
                # with torch.cuda.amp.autocast(enabled=(device=='cuda')): # If using mixed precision
                outputs = model(audio_feat, video_feat, pers_feat)
                loss = criterion(outputs, labels)
    ...
    # print(f"  Eval batches processed: {processed_batches}") # Optional debug print
    if num_samples == 0:
...
if __name__ == '__main__':
    ...
    # --- Paths ---
    DATA_ROOT_PATH = "/Users/kaushaldamania/deepl/MPDD-Elderly" # Your data root
    MODEL_SAVE_PATH = './best_late_fusion_transformer_cls_v3.pth' # New name for this version
    ...
    num_workers = 0 # Often 0 is best for MPS, adjust if needed
    # Use persistent_workers=True if num_workers > 0 for potential speedup
    # Be cautious with drop_last=True if your dataset size is small
    ...
    for epoch in range(NUM_EPOCHS_FINAL):
        ...
        # Save if F1 is extracted and better than the current best
